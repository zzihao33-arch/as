...
隔离候选 `codex/pickup-production-candidate` 已从 `ca05db3` 构造完成，代码合并点 `fa32f91`。集成日志和 TYG 修复得到保留；提货凭证迁移顺延为 019/020。本地前端 54/54、后端 221/221、20 份迁移校验、类型检查、构建、浏览器合成回归及扫描脚本语法检查均通过。

1. 将该候选部署到测试环境，重新执行真实 API、浏览器、恶意文件拒绝、权限、数据清理和整机重启验收。
