...
The employee-document v1 candidate uses migrations `017_add_warehouse_ui_operations.sql` and `018_add_pickup_documents.sql` on this test release line. The version numbers differ from the standalone candidate because this release line already uses 014 for customer profiles, 015 for legacy pickup attachments, and 016 for label retention. Migration 018 keeps the existing `air_pickup_document_assets` table and its rows intact; checked v2 uploads use `air_pickup_document_assets_v2`. Existing attachment reads remain available while legacy writes are disabled. No existing role receives new permissions. Verify the target's migration history and checksums before rollout, and keep `PICKUP_DOCUMENTS_ENABLED=false` until the real checker and access tests pass.
...
17. `017_add_warehouse_ui_operations.sql` adds the durable UI operation ledger required by retryable checked document uploads.
18. `018_add_pickup_documents.sql` adds the checked v2 upload and preview metadata, extends the event/operation enums, and leaves the legacy attachment table and rows intact.
