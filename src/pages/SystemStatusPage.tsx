  ...
  return (
    <section className="cmhub-page cmhub-system-page cmhub-stitch-page" aria-labelledby="system-status-title">
      <header className="cmhub-stitch-heading cmhub-system-heading">
        ...
        <Button type="primary" icon={<RefreshCw size={16} />} loading={checking} onClick={() => void detect()}>重新检测全部服务</Button>
      </header>

      <div className="cmhub-system-summary">
      ...
      <div className="cmhub-system-status-grid">
        {statusItems.map(item => {
          const Icon = item.icon;
          return <article key={item.key} data-state={item.state}>
      ...
      <div className="cmhub-system-body">
        <main>
          <section className="cmhub-system-panel">
            <header><h2>硬件与本地存储</h2><Button type="text" onClick={() => void detect()}>重新检测</Button></header>
            <div className="cmhub-system-hardware">
              <div><Printer size={24} /><span><small>当前打印机</small><strong>{selectedPrinter || '自动选择可直打打印机'}</strong><Tag color={selectedPrinter ? 'green' : 'orange'}>{selectedPrinter ? '已配置' : '待确认'}</Tag></span></div>
              <div><span><small>本地缓存使用</small><strong>{cacheEstimate}%</strong></span><Progress percent={cacheEstimate} showText={false} /><small>浏览器会按模块容量策略自动维护缓存</small></div>
            </div>
          </section>

          <section className="cmhub-system-panel">
            <header><h2>最近异常摘要</h2><Tag bordered={false}>本次检测</Tag></header>
        ...
        <aside>
          <section className="cmhub-system-panel cmhub-system-environment">
          ...
          <section className="cmhub-system-advisory"><HardDrive size={21} /><div><strong>现场排查建议</strong><p>{qzConnected ? 'QZ Tray 已连接。如打印无响应，请检查打印机纸张和系统队列。' : '先启动 QZ Tray，再回到本页重新检测。'}</p></div></section>
          <section className="cmhub-system-advisory is-info"><PackageCheck size={21} /><div><strong>面单服务</strong><p>私有面单仅通过授权工作站下载，不会在浏览器中公开链接。</p></div></section>
        </aside>
      </div>
    </section>
  );
